...
public class BlenderAdd implements Filter{
    ...
    public void apply(AbstractNode node) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                node.output1.getContent().setRGB(x,y,Math.min(256,RGBUtils.redComponent(color1)+RGBUtils.redComponent(color2)) << 16
                | Math.min(256,RGBUtils.greenComponent(color1)+RGBUtils.greenComponent(color2)) << 8
                | Math.min(256,RGBUtils.blueComponent(color1)+RGBUtils.blueComponent(color2)));
            }
        }
    }
